...
describe('Processo Jurídico Formatter', () => {
  test('should format Processo Jurídico with mask', () => {
    expect(format('')).toBe('');
  ...
  test(`should NOT add digits after the Processo Jurídico length (${LENGTH})`, () => {
    expect(format('00020802520125150049123123')).toBe(
      '0002080-25.2012.515.0049'
    );
  });

  test('should remove all non numeric characters', () => {
    expect(format('0002080@$25201%!@2515.%0049123123')).toBe(
      '0002080-25.2012.515.0049'
    );
  });
});
